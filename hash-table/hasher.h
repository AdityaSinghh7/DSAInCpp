...
struct Hasher {
  const string name;
  Hasher(const char *nm) : name(nm) {}
  virtual size_t hash(string key, int N) const = 0;
};

struct MultHasher : public Hasher {
  MultHasher() : Hasher("MultHasher") {}

  size_t hash(string key, int N) const {
    size_t result = 1;
    int len = key.length();
    for (int i = 0; i < len; ++i) result *= key[i];
    return result % N;
  }
};
...
struct WeissHasher : public Hasher {
  WeissHasher() : Hasher("WeissHasher") {}
...
struct PJWHasher : public Hasher {
  PJWHasher() : Hasher("PJWHasher") {}
...
struct GoodrichHasher : public Hasher {
  GoodrichHasher() : Hasher("GoodrichHasher") {}
 ...
 */
struct PreissHasher : public Hasher {
  PreissHasher() : Hasher("PreissHasher") {}
  ...
  size_t hash(string key, int N) const {
    ...
    // How much to shift hash by, per char hashed
    constexpr size_t PREISS_HASH_MASK = ~0U << (WORD_WIDTH - PREISS_HASH_SHIFT);
    // Mask for extracting top bits
    size_t hashValue = 0;
    for (string::const_iterator i = key.begin(); i != key.end(); i++)
      hashValue = (hashValue & PREISS_HASH_MASK) ^
                  (hashValue << PREISS_HASH_SHIFT) ^ (unsigned)*i;
    return hashValue % N;
  }
};
...
struct Weiss1Hasher : public Hasher {
  Weiss1Hasher() : Hasher("Weiss1Hasher") {}
 ...
 */
struct Weiss2Hasher : public Hasher {
  Weiss2Hasher() : Hasher("Weiss2Hasher") {}
...
struct STLHasher : public Hasher {
  STLHasher() : Hasher("STLHasher") {}
...
struct ChatGPTHasher : public Hasher {
  ChatGPTHasher() : Hasher("ChatGPTHasher") {}
